...
const Player = () => {
  ...
  return(
    <div className={cssContainer}>
      ...
      />
      <button onClick={()=> handleTogglePlay(vidRef.current.paused)} type="button">{paused? "Play":"Pause"}</button>
      <button onClick={()=>handleToggleMute()} type="button">{muted ? "Unmute": "Mute"}</button>
      <input value={time} onChange={(e)=>{
        vidRef.current.currentTime = e.target.value}} max={vidRef.current.duration} min={0} type="range"/>
